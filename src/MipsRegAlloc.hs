...
-- test1 :: Bool
-- test1 = regAllocInsts 4 8 (map (\x -> ('$':(show x))) [8..16]) [Label "fun0",ADDI "$sp" "$sp" "-8",SW "$ra" "4" "$sp",SW "$fp" "0" "$sp",XOR "$fp" "$sp" "$0",XORI "plus1_2" "$0" "4",XORI "plus1_4" "$0" "4",XORI "plus2_5" "$0" "4",ADD "plus2_3" "plus1_4" "plus2_5",ADD "ret_fun0_1" "plus1_2" "plus2_3",XOR "$v0" "ret_fun0_1" "$0",LW "$fp" "0" "$sp",LW "$ra" "4" "$sp",ADDI "$sp" "$sp" "8",JR "$ra"]
--         ==
--         [Label "fun0",ADDI "$sp" "$sp" "-8",SW "$ra" "4" "$sp",SW "$fp" "0" "$sp",XOR "$fp" "$sp" "$0",XORI "$8" "$0" "4",XORI "$9" "$0" "4",XORI "$11" "$0" "4",ADD "$10" "$9" "$11",ADD "$12" "$8" "$10",XOR "$v0" "$12" "$0",LW "$fp" "0" "$sp",LW "$ra" "4" "$sp",ADDI "$sp" "$sp" "8",JR "$ra"]

-- test2 :: Bool
-- test2 = regAllocInsts 4 8 (map (\x -> ('$':(show x))) [8..16]) [Label "fun0",ADDI "$sp" "$sp" "-8",SW "$ra" "4" "$sp",SW "$fp" "0" "$sp",XOR "$fp" "$sp" "$0",XORI "plus1_2" "$0" "4",XORI "plus1_4" "$0" "4",XORI "plus2_5" "$0" "4",ADD "plus2_3" "plus1_4" "plus2_5",ADD "ret_fun0_1" "plus1_2" "plus2_3",XOR "$v0" "ret_fun0_1" "$0",LW "$fp" "0" "$sp",LW "$ra" "4" "$sp",ADDI "$sp" "$sp" "8",JR "$ra"]
--         ==
--         [Label "fun0",ADDI "$sp" "$sp" "-8",SW "$ra" "4" "$sp",SW "$fp" "0" "$sp",XOR "$fp" "$sp" "$0",XORI "$8" "$0" "4",XORI "$9" "$0" "4",XORI "$11" "$0" "4",ADD "$10" "$9" "$11",ADD "$12" "$8" "$10",XOR "$v0" "$12" "$0",LW "$fp" "0" "$sp",LW "$ra" "4" "$sp",ADDI "$sp" "$sp" "8",JR "$ra"]

-- test3 = getLiveness [Label "fun0",ADDI "$sp" "$sp" "-8",SW "$ra" "4" "$sp",SW "$fp" "0" "$sp",XOR "$fp" "$sp" "$0",XOR "var_2" "$a0" "$0",XORI "plus1_3" "$0" "4",XORI "plus1_5" "$0" "4",XOR "plus2_6" "var_2" "$0",ADD "plus2_4" "plus1_5" "plus2_6",ADD "ret_fun0_1" "plus1_3" "plus2_4",XOR "$v0" "ret_fun0_1" "$0",LW "$fp" "0" "$sp",LW "$ra" "4" "$sp",ADDI "$sp" "$sp" "8",JR "$ra"]

-- (inSet, outSet, kls, gls) = test3

-- test4 = getInterferenceSet outSet kls

-- test5 = getSRegNames kls gls

-- test6 :: Graph
-- test6 = (test5, test4)
-- test6b = map (\x -> (x, noOfEdgesForNode (test5, test4) x)) (S.toList test5)

-- test7 = removeNodeAndEdges test6 "plus1_3"
-- test8 = removeNodeAndEdges test7 "plus2_4"

-- test9 = noOfEdgesForNode test6 "plus1_5"
-- test10 = noOfEdgesForNode test7 "plus1_5"
-- test11 = noOfEdgesForNode test8 "plus1_5"


-- test12 = colorGraphSimplify (map (\x -> '$':(show x)) [8..12]) test6


-- test13 = getLiveness  [Label "fun0",ADDI "$sp" "$sp" "-8",SW "$ra" "$sp" "4",SW "$fp" "$sp" "0",XOR "$fp" "$sp" "$0",XOR "var_2" "$a0" "$0",XOR "var_3" "$a1" "$0",XOR "var_4" "$a2" "$0",XOR "var_5" "$a3" "$0",XOR "mult1_69" "var_2" "$0",SW "mult1_69" "$sp" "0",XOR "mult1_8" "var_3" "$0",XOR "mult1_1011" "var_4" "$0",SW "mult1_1011" "$sp" "-4",XOR "mult2_11" "var_5" "$0",LW "mult1_1013" "$sp" "-4",MUL "mult2_9" "mult1_1013" "mult2_11",MUL "mult2_7" "mult1_8" "mult2_9",LW "mult1_615" "$sp" "0",MUL "ret_fun0_1" "mult1_615" "mult2_7",XOR "$v0" "ret_fun0_1" "$0",LW "$fp" "$sp" "0",LW "$ra" "$sp" "4",ADDI "$sp" "$sp" "8",JR "$ra"]

-- (inSet2, outSet2, kls2, gls2) = test13
